...
Иногда объявленный маршрут по регулярному выражению может подходить под переданный URI, но при этом по смыслу вообще не подходит.
...
В данном примере приведены три способа вызвать функцию для обработки маршрута. Первый и второй способ - передача `callback`'а,
 т.е. массива с объектом (имя класса или заранее созданный экземпляр объекта) и именем его (публичного, т.е. `public`) метода.
 В него автоматически будет передан обрабатываемый URI в качестве единственного параметра.

### Пример 1 {#example1}

Предположим, мы хотим маршрутизировать адреса, содержащие географические пути, например `<country>(/<region>(/<city>(...)))`.
 Маршрут с неограниченной вложенностью невозможно разобрать стандартным маршрутом, поэтому необходимо использовать функции:

	// объявляем маршрут
	Route::set('geo', array('Geo', 'route'), 'geo/<path>');

	// собственно класс с логикой обработки маршрута
	class Geo {
		public static function route($uri) {
			// 1. Готовим шаблон для проверки адреса
			$pattern = Route::compile('geo/<path>', array('path' => '.*'));
			// 2. Проверяем на соответствие шаблону
			...
			else
			{
	            ...
	            {
	                // проверяем корректность каждого ГЕО-сегмента
	                if ( ! Geo::object_exists($segment, $parents))
	                {
	                    // объекта нет, маршрут не подходит
	                    // как вариант, можно подставлять какой-то дефолтный адрес
	                    return FALSE;
	                }

	                $result['objects'][] = $segment;
	                $parents[] = $segment;
	            }

	            // 4. Можно выполнять произвольные действия по итогам вычисления
	            Cookie::set('geo-region', $result['objects'][0]);

	            return $result;
			}
		}

		// здесь проверяем ГЕО-объект на существование и принадлежность родителям
		public static function object_exists($name, $parent = NULL) {...}
	}

[!!] Главное требование - функция обработки должна принимать на входе проверямый URI, а возвращать массив сегментов. Любой другой
 тип данных будет означать, что маршрут не подходит.

[!!] Обратили внимание на третий параметр в `Route::set()`? Мы передали туда строку `geo/<path>`, чтобы маршрут мог использовать
 этот шаблон для [обратной маршрутизации](basic/routing/reverse). Если создавать URI из маршрута мы не планируем, то можно
 ограничиться только первыми двумя аргументами.

Данный пример описывает работу со статическим обработчиком, но и остальные два способа в принципе аналогичны.

## Где лучше объявлять? {#places}
